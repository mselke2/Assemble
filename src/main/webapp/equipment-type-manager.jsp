...
<div class="panel">
  ...
  <c:if test="${requestingUser.clearanceAtLeast('admin')}">
    <p class="warning" style="color: red">Warning: Deleting equipment types will delete all jobs associated with the
      deleted equipment type.</p>
    ...
    <c:if test="${not empty equipmentTypes}">
      <c:forEach var="equipmentType" items="${equipmentTypes}">
        <tr resource-id="${equipmentType.id}">
          ...
          <td><input type="text" name="description" maxlength="50" required value="${equipmentType.description}"
