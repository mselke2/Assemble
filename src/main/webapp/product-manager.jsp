...
<div class="panel">
    ...
    <br><br>
  <c:if test="${requestingUser.clearanceAtLeast('admin')}"><p class="warning">WARNING: Editing any products will DELETE ALL JOBS associated with that product.</p></c:if>
    <table>
        <tr>
            <th></th>
        ...
        <c:if test="${not empty products}">
            <c:forEach var="product" items="${products}">
                <tr resource-id="${product.id}">
                  ...
                  <td class="data"><input type="text" name="description" required class="description" pattern="^.{1,50}$"  value="${product.description}" <c:if test="${!requestingUser.clearanceAtLeast('admin')}">disabled</c:if>></td>

                  <td class="data"><input type="number" name="duration" required class="duration" min="1" value="${product.minutesDuration}" <c:if test="${!requestingUser.clearanceAtLeast('admin')}">disabled</c:if>></td>

                  <td class="resource-count data"><input type="number" name="personnelCount" required class="personnel-count" min="1" value="${product.targetPersonnelCount}" <c:if test="${!requestingUser.clearanceAtLeast('admin')}">disabled</c:if>></td>
        ...
        <c:if test="${not empty inventoryTypes}">
            <c:forEach var="inventoryType" items="${inventoryTypes}">
                <li>
                  ...
                  <input type="number" inventory-type-id="${inventoryType.id}" min="0" class="inventory-count" <c:if test="${!requestingUser.clearanceAtLeast('admin')}">disabled</c:if>>
                </li>
            </c:forEach>
        </c:if>
        </ul>
        <button class="editor-submit">Ok</button>
      <c:if test="${requestingUser.clearanceAtLeast('admin')}"><button class="editor-cancel">Cancel</button></c:if>
    </div>
</div>
        ...
        <c:if test="${not empty equipmentTypes}">
            <c:forEach var="equipmentType" items="${equipmentTypes}">
                <li>
                  ...
                  <input type="number" equipment-type-id="${equipmentType.id}" min="0" class="equipment-count" <c:if test="${!requestingUser.clearanceAtLeast('admin')}">disabled</c:if>>
                </li>
            </c:forEach>
        </c:if>
        </ul>
