          ...
          <div id="timeline-lanes">
            ...
            <c:if test="${not empty jobs}">
              <c:forEach var="job" items="${jobs}">
                <div class="job-entry" job-id="${job.id}"
                     style="--start-time: ${job.startTime.getHours() + job.startTime.getMinutes() / 60}; --end-time: ${job.projectedEndTime.getHours() + job.projectedEndTime.getMinutes() / 60}; --lane: ${job.lineNumber};">
                  <p>${job.id}</p>
                </div>
              </c:forEach>
            </c:if>
        ...
        <div class="job-info">
          ...
          <div id="job-form" hidden>
            <input type="hidden" name="date" id="date"
                   value="${param.d.substring(0, 4)}-${param.d.substring(4, 6)}-${param.d.substring(6)}">
            ...
            <input type="time" name="start-time" id="start-time"
                   <c:if test="${!requestingUser.clearanceAtLeast('editor')}">disabled</c:if>><br>
            ...
            <input type="number" min="1" name="line-num" id="line-num"
                   <c:if test="${!requestingUser.clearanceAtLeast('editor')}">disabled</c:if>><br>
